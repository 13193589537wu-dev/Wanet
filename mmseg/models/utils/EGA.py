...
def gauss_kernel(channels=3, cuda=True, device=None):
...
def upsample(x, channels):
    ...
    return conv_gauss(x_up, 4 * gauss_kernel(channels, cuda=True, device=x.device))


def make_laplace(img, channels=None):
    if channels is None:
        channels = img.shape[1]  # 动态获取输入通道数
    filtered = conv_gauss(img, gauss_kernel(channels, cuda=True, device=img.device))
...
def make_laplace_pyramid(img, level, channels):
    ...
    for _ in range(level):
        filtered = conv_gauss(current, gauss_kernel(channels, cuda=True, device=img.device))
        down = downsample(filtered)
        up = upsample(down, channels)
...
class EGA(nn.Module):
    ...
    def forward(self, edge_feature, x, pred):
        ...
        # boundary attention
        edge_pred = make_laplace(pred)  # 使用动态通道数
