...
from ..utils.AgentAttention import AgentAttention
#TODO,inhe inter huancheng in chanel
# @HEADS.register_module()
# class VitGuidanceHead(BaseDecodeHead):
#     """
#     VitGuidanceHead 用于知识蒸馏任务，student 学习 teacher 的中间特征图。
#     """
#
#     def __init__(self,
...
#         self.channelsDefault = [base_channels * 4, 320, 512]  # student 输出通道映射
#
...
#
#         # 单独为 student 特征构建一个 fusion layer
#         self.student_fusion = ConvModule(
#             in_channels=256,
#             out_channels=256,
#             kernel_size=3,
#             padding=1,
...
#             param.requires_grad = False
#
#         # 判断是否加载预训练模型（兼容 init_cfg=None）
#         if self.init_cfg is not None and self.init_cfg.get('type') == 'Pretrained':
#             pretrained = self.init_cfg.get('checkpoint')
#             print(f"Loading VitGuidanceHead weights from {pretrained}")
#             state_dict = torch.load(pretrained)['state_dict']
#             self.teacher_backbone.load_state_dict(state_dict, strict=False)
#             self.teacher_head.load_state_dict(state_dict, strict=False)
#
...
#         if vitfeature[0].size(-2) != stu_channel_align.size(-2) or \
#                 vitfeature[0].size(-1) != stu_channel_align.size(-1):
...
#         else:
#             sampled_decoder_feature = stu_channel_align
#
#         stu_fea = self.align_scale(sampled_decoder_feature, feature)
#         stu_decoder_feature = self.student_fusion(stu_fea)  # ✅ 使用自定义 fusion
#         stu_decoder_logits = self.teacher_head.cls_seg(stu_decoder_feature)
#
#         feat_s = []
#         feat_s.append(stu_decoder_logits)
#         feat_s.append(stu_decoder_feature)
#         feat_s.append(self.convstage4(x_guidance_feature[1][1]))
#         feat_s.append(self.convstage3(x_guidance_feature[1][2]))
#
